---
title: "World Universities Rankings Exploration"
...
```{r cwur_data_plotly_world}
cwurCountryRankYear <- function(nYear) {
  cwurData %>% filter(year == nYear) %>% group_by(country) %>%
  summarise(nr = length(world_rank), minw=min(world_rank), maxw=max(world_rank), avgw=round(mean(world_rank),0)) %>%
  select(country, nr, minw, maxw, avgw) %>% ungroup() -> ccwur
...
plot_geo(ccwur, locationmode = 'country names') %>%
  ...
  layout(
    title = with(ccwur, paste('Number of universities in top(',nYear,')<br>Source:<a href="http://cwur.org/">Council of World University Ranking</a>')),
    geo = g
  )
}
...
We apply this function for each of the CWUR data years (from 2012 to 2015).


###2012
```{r fig.width=9, fig.height=6,cwur_country_year2012}
cwurCountryRankYear(2012)
```

###2013
```{r fig.width=9, fig.height=8,cwur_country_year2013}
cwurCountryRankYear(2013)
```

###2014
```{r fig.width=9, fig.height=8,cwur_country_year2014}
cwurCountryRankYear(2014)
```

###2015
```{r fig.width=9, fig.height=8,cwur_country_year2015}
cwurCountryRankYear(2015)
```
...
All these values are for a corresponding year between 2005 and 2015. As these values are all ranking between 0 and 100, it will be relevant to represent them as Spider-Web graphs.
...
The minimum Shanghai Total Score is now `r min(shanghaiDataCld$total_score)` and the maximum is still
`r max(shanghaiDataCld$total_score)`.
...
```{r shanghai_data_plotly_world}


shanghaiCountryRankYear <- function(nYear) {
  scData %>% filter(year == nYear) %>% group_by(country) %>%
  summarise(nr = length(total_score), minw=min(total_score), maxw=max(total_score), avgw=round(mean(total_score),0)) %>%
  select(country, nr, minw, maxw, avgw) %>% ungroup() -> swur
...
plot_geo(swur, locationmode = 'country names') %>%
  ...
  layout(
    title = with(swur, paste('Number of universities (',nYear,')<br>Source:<a href="http://www.shanghairanking.com">Shanghai Academic World University Rankings</a>')),
    geo = g
  )
}
...
Press on the tab years to see the Shanghai countries ranks for each year.

###2005
```{r fig.width=9, fig.height=8,swc2005}
shanghaiCountryRankYear(2005)
```

###2006
```{r fig.width=9, fig.height=8,swc2006}
shanghaiCountryRankYear(2006)
```

###2007
```{r fig.width=9, fig.height=8,swc2007}
shanghaiCountryRankYear(2007)
```

###2008
```{r fig.width=9, fig.height=8,swc2008}
shanghaiCountryRankYear(2008)
```

###2009
```{r fig.width=9, fig.height=8,swc2009}
shanghaiCountryRankYear(2009)
```

###2010
```{r fig.width=9, fig.height=8,swc2010}
shanghaiCountryRankYear(2010)
```

###2011
```{r fig.width=9, fig.height=8,swc2011}
shanghaiCountryRankYear(2011)
```


###2012
```{r fig.width=9, fig.height=8,swc2012}
shanghaiCountryRankYear(2012)
```


###2013
```{r fig.width=9, fig.height=8,swc2013}
shanghaiCountryRankYear(2013)
```


###2014
```{r fig.width=9, fig.height=8,swc2014}
shanghaiCountryRankYear(2014)
```


###2015
```{r fig.width=9, fig.height=8,swc2015}
shanghaiCountryRankYear(2015)
```
...
We will look to the aggregated values per country (top 10 countries) for the same factors studied also for the top 10 universities,
i.e. Teaching, International Outlook, Research, Citations, Industry Income and Overall (total_score).  
...
```{r fig.width=10, fig.height=9,the_countries_boxplots,warning=FALSE}
...
Note: the total score is highly affected by missing values. Industry income (income) and International Outlook (international) are
as well partially affected.

Let's represent now the number of universities per countries on a World map.
We will use as value  for aggregation the total score.

```{r the_data_plotly_world}
timesData$total_score = as.numeric(as.character(timesData$total_score))
#replace with 0 the missing total_score values - this will affect the aggregated values

timesHigherEducationCountryRankYear <- function(nYear) {


  timesData %>% filter(year == nYear) %>% group_by(country) %>%
  summarise(nr = length(total_score), minw=min(total_score), maxw=max(total_score), avgw=round(mean(total_score),0)) %>%
  select(country, nr, minw, maxw, avgw) %>% ungroup() -> ther
...
plot_geo(ther, locationmode = 'country names') %>%
  ...
  colorbar(title = 'Number of\nuniversities', tickprefix = '') %>%
  layout(
    title = with(ther, paste('Number of universities (',nYear,')<br>Source:<a href="https://www.timeshighereducation.com/world-university-rankings">Times Higher Education World University Ranking</a>')),
    geo = g
  )
}
...
Press on the tab years to see the Times Higher Education ranks per countries for each year.

###2011
```{r fig.width=9, fig.height=8,the_wm_2011,warning=FALSE}
timesHigherEducationCountryRankYear(2011)
```

###2012
```{r fig.width=9, fig.height=8,the_wm_2012,warning=FALSE}
timesHigherEducationCountryRankYear(2012)
```

###2013
```{r fig.width=9, fig.height=8,the_wm_2013,warning=FALSE}
timesHigherEducationCountryRankYear(2013)
```

###2014
```{r fig.width=9, fig.height=8,the_wm_2014,warning=FALSE}
timesHigherEducationCountryRankYear(2014)
```

###2015
```{r fig.width=9, fig.height=8,the_wm_2015,warning=FALSE}
timesHigherEducationCountryRankYear(2015)
```

###2016
```{r fig.width=9, fig.height=8,the_wm_2016,warning=FALSE}
timesHigherEducationCountryRankYear(2016)
```
...
As we already explained, THE contains additional metrics (Number of FTE Students, Number of Students per Staff,
International Students, Female-male ratio). The first three variables can be used directly, for female-male ratio we will need to process
the data.
...
We can see that the Caltech has the smaller number of students, UCLA the largest, Yale University has the largest percent of female
students (50%), ETH Zurich the smallesr (but above 30%), Yale has the smallest Student-staff ratio (less than 5) while University of
California at Berkeley the largest (~16), the percentage of foreign students is largest at University College London (above 50%) and
it is the smalles at University of California at Berkeley (~15%).



#**Romanian Universities in World Rankings**

I am a Romanian so I would like to see how Romanian Universities are placed in the World Rankings. Unfortunatelly, just
a few universities are present, they are very low in the hierarchy, and this only in recent years.
...
```
There are just two of them, `University of Bucharest` (appearing in 2014 and 2015) and `Babeș-Bolyai University` (in 2015).
The score is around 44, on a scale from 0 to 100 (Harvard University has usually 100).

Shanghai (AWUR) does not contain any Romanian university.

Let's look now to the Romanian universities present in THE hierarchy.
...
```
Four universities made the hierarchy in 2016 (the first time).

Let's represent these data from THE about Romanian Universities against the same year top 5 universities data.
...
industry income but typically the performance is poor for research and generally poor for citations. The conclusion is simple.
For these Romanian Universities best placed in 2016 in THE World University Rankings (ranking in 501-600
range and601-800 range) there is a lot to be improved (especially in research) in order to approach the performance of the top
...
University of Bucharest, Babes-Bolyai University (Cluj) and Alexandru Ioan Cuza University (Iasi) have all larger numbers of
full-time students than the top 5 World Universities. Only Timisoara University (a much smaller town) has a smaller number of
students, still, one of the largest compared with top 5 World Universities. The student-staff ratio is much higher than the
top 5 for all Romanian universities (smaller than 20 for West University of Timisoara) with Alexandru Ioan Cuza (Iasi) topping
at over 30 students per staff. The number of international students is small for all 4 Romanian Universities in THE Rankings,
with values between 2 and 5%. Alexandru Ioan Cuza University (Iasi) has almost 5% international students. As per the female-male
ratio, here we see something characteristic to former communist countries but also a sign of modernity: female-male ratio is
higher than 60% for all universities (65% for University of Bucharest), for two reasons: historically, female-male ratio is high,
due to egalitarian tradition in former communist countries; as well, female ratio tend to be higher in non-technical faculties and
all these Universities does not include technical faculties (traditionally segregated in former communist countries to separate
higher education institutions: here the female-male ratio is smaller).
...
Let's process the information about Education expenditure. There are ``r length(unique(educationExpenditure$country))`` countries
(or suprastatal entities, like OECD, for reference) analyzed.
As well, there are ``r length(unique(educationExpenditure$institute_type))`` different institute types included:  
``r unique(educationExpenditure$institute_type)``.
As for the direct expenditure types, there are ``r length(unique(educationExpenditure$direct_expenditure_type))`` diferent values:
``r unique(educationExpenditure$direct_expenditure_type)``.
...
Before presenting the `institute_type` and `direct_expenditure_type` to the function, we convert the factor in characters and trim the
white spaces (so that we can use easier the filters).
...
educationExpenditureRatioYear <- function(nYear,nType, nDirectExpenditureType) {

    educationExpenditureFlatten %>% filter(year == nYear,
    ...
    add_trace(z = ~ratio, color = ~ratio, colors = 'Spectral',
        text = ~hover, locations=~country, marker = list(line = l)) %>%
    colorbar(title = 'Ratio', tickprefix = '%') %>%
    layout(
        title = with(eef, paste('Education expenditure ratio (<b>',nYear,'</b>);<br>Institution type:<b>',nType,'</b>; Expenditure type:<b>',nDirectExpenditureType,'</b>)')),
        geo = g
    )
}
```  
...
Let's show now the Public expenditures for All institutions (World map for all the years). Click on <font color="blue">year tabs</font>
to see the data for each year.  

The public expenditures ratio for all institutions are highest in 1995 in Norway (6.8%), Sweden and Finland (6.6%), Denmark (6.4%),
followed by France & Canada (5.8%), Portugal (5.4%) and New Zeeland or Austria (5.3%). US has only 5%, being below  Poland (5.2%).
Norway, Sweden, Denmark, Finland keep their leading position from 1995 to 2011, with the top values for 2011 being for Denmark (7.5%)
and Norway (7.3%). US, Germany, UK have values in those years between 4.4% and 5.6%. In 2011 US is behind countries like Brazil (5.9%)
or even Mexic (5%).

####1995
```{r fig.width=9, fig.height=6,eef_a_pu_1995,warning=FALSE}
educationExpenditureRatioYear(1995,"All Institutions", "Public")
```

####2000
```{r fig.width=9, fig.height=6,eef_a__pu_2000,warning=FALSE}
educationExpenditureRatioYear(2000,"All Institutions","Public")
```

####2005
```{r fig.width=9, fig.height=6,eef_a__pu_2005,warning=FALSE}
educationExpenditureRatioYear(2005,"All Institutions","Public")
```

####2009
```{r fig.width=9, fig.height=6,eef_a__pu_2009,warning=FALSE}
educationExpenditureRatioYear(2009,"All Institutions","Public")
```

####2010
```{r fig.width=9, fig.height=6,eef_a__pu_2010,warning=FALSE}
educationExpenditureRatioYear(2010,"All Institutions","Public")
```


####2011
```{r fig.width=9, fig.height=6,eef_a__pu_2011,warning=FALSE}
educationExpenditureRatioYear(2011,"All Institutions","Public")
```
...
Let's inspect now the Private expenditures for All institutions (World map for all the years). Click on <font color="blue">year tabs</font>
to see the data for each year.  

There are no data about private expenditure for education for the years 1995 to 2010. We only have data for 2011.
For this year, South Korea is leading with 2.8% ratio, followed by Chile (2.5%), United States (2.2%), Japan (1.6%),
Australia (1.5%), New Zeeland (1.2%) and Mexic (1.1%).


####1995
```{r fig.width=9, fig.height=8,eef_a_pr_1995,warning=FALSE}
educationExpenditureRatioYear(1995,"All Institutions", "Private")
```

####2000
```{r fig.width=9, fig.height=8,eef_a__pr_2000,warning=FALSE}
educationExpenditureRatioYear(2000,"All Institutions","Private")
```

####2005
```{r fig.width=9, fig.height=8,eef_a__pr_2005,warning=FALSE}
educationExpenditureRatioYear(2005,"All Institutions","Private")
```

####2009
```{r fig.width=9, fig.height=8,eef_a__pr_2009,warning=FALSE}
educationExpenditureRatioYear(2009,"All Institutions","Private")
```

####2010
```{r fig.width=9, fig.height=8,eef_a__pr_2010,warning=FALSE}
educationExpenditureRatioYear(2010,"All Institutions","Private")
```


####2011
```{r fig.width=9, fig.height=8,eef_a_pr_2011,warning=FALSE}
educationExpenditureRatioYear(2011,"All Institutions","Private")
```

###Total expenditure {.tabset .tabset-fade .tabset-pills}


Let's inspect now the Total expenditures for All institutions (World map for all the years). Click on <font color="blue">year tabs</font>
to see the data for each year.  

The Total expenditure for All institutions data is available only for 2011. In 2011 the countries with largest Total expenditure
for All institutions are Denmark (7.9%), Iceland (7.7%), South Korea (7.6%), New Zeeland (7.5%), United States (6.9%) and
Finland (6.5%).


####1995
```{r fig.width=9, fig.height=8,eef_a_t_1995,warning=FALSE}
educationExpenditureRatioYear(1995,"All Institutions", "Total")
```

####2000
```{r fig.width=9, fig.height=8,eef_a__t_2000,warning=FALSE}
educationExpenditureRatioYear(2000,"All Institutions","Total")
```

####2005
```{r fig.width=9, fig.height=8,eef_a__t_2005,warning=FALSE}
educationExpenditureRatioYear(2005,"All Institutions","Total")
```

####2009
```{r fig.width=9, fig.height=8,eef_a__t_2009,warning=FALSE}
educationExpenditureRatioYear(2009,"All Institutions","Total")
```

####2010
```{r fig.width=9, fig.height=8,eef_a__t_2010,warning=FALSE}
educationExpenditureRatioYear(2010,"All Institutions","Total")
```


####2011
```{r fig.width=9, fig.height=8,eef_a_t_2011,warning=FALSE}
educationExpenditureRatioYear(2011,"All Institutions","Total")
```
...
Click on <font color="blue">year tabs</font> to see the data for each year.  

North European countries are in the top starting with 1995. Sweden (4.4%), Finland (4.2%), Norway (4.1%) are leading the World
and European countries. Follows France (4.1%), Switzerland (4.1%) and Canada (4.0%). UK has only 3.8% and US 3.5%. In the
following years the top keeps the Nordics in the top but we see New Zeeland on first place with 4.6% (2000), Iceland with 5.2%
(2005, 2009). In te following years Norway is leading with 5.1% (2010) and 4.9% (2011).

####1995
```{r fig.width=9, fig.height=8,eef_es_pu_1995,warning=FALSE}
educationExpenditureRatioYear(1995,"Elementary and Secondary Institutions", "Public")
```

####2000
```{r fig.width=9, fig.height=8,eef_es_pu_2000,warning=FALSE}
educationExpenditureRatioYear(2000,"Elementary and Secondary Institutions","Public")
```

####2005
```{r fig.width=9, fig.height=8,eef_es_pu_2005,warning=FALSE}
educationExpenditureRatioYear(2005,"Elementary and Secondary Institutions","Public")
```

####2009
```{r fig.width=9, fig.height=8,eef_es_pu_2009,warning=FALSE}
educationExpenditureRatioYear(2009,"Elementary and Secondary Institutions","Public")
```

####2010
```{r fig.width=9, fig.height=8,eef_es_pu_2010,warning=FALSE}
educationExpenditureRatioYear(2010,"Elementary and Secondary Institutions","Public")
```


####2011
```{r fig.width=9, fig.height=8,eef_es_pu_2011,warning=FALSE}
educationExpenditureRatioYear(2011,"Elementary and Secondary Institutions","Public")
```
...
For the years from 1995 to 2010, there are no data for Elementary and Secondary Institutions (private expenditures).
For the year 2011, the first place belongs to South Korea (0.8%). South Korea is followed by Chile (0.7%), Australia,
New Zeeland and Mexic (0.6%). In Europe, Switzerland has the best ratio (0.5%).

####1995
```{r fig.width=9, fig.height=8,eef_es_pr_1995,warning=FALSE}
educationExpenditureRatioYear(1995,"Elementary and Secondary Institutions", "Private")
```

####2000
```{r fig.width=9, fig.height=8,eef_es_pr_2000,warning=FALSE}
educationExpenditureRatioYear(2000,"Elementary and Secondary Institutions","Private")
```

####2005
```{r fig.width=9, fig.height=8,eef_es_pr_2005,warning=FALSE}
educationExpenditureRatioYear(2005,"Elementary and Secondary Institutions","Private")
```

####2009
```{r fig.width=9, fig.height=8,eef_es_pr_2009,warning=FALSE}
educationExpenditureRatioYear(2009,"Elementary and Secondary Institutions","Private")
```

####2010
```{r fig.width=9, fig.height=8,eef_es_pr_2010,warning=FALSE}
educationExpenditureRatioYear(2010,"Elementary and Secondary Institutions","Private")
```


####2011
```{r fig.width=9, fig.height=8,eef_es_pr_2011,warning=FALSE}
educationExpenditureRatioYear(2011,"Elementary and Secondary Institutions","Private")
```
...
The Total expenditure for Elementary and Secondary Institutions is available only for 2011. In this year the top
countries are New Zeeland (5.4%), Iceland (4.9%), United Kingdom (4.7%), Ireland (4.6%), Denmark and Belgium (both
with 4.4%).

####1995
```{r fig.width=9, fig.height=8,eef_es_t_1995,warning=FALSE}
educationExpenditureRatioYear(1995,"Elementary and Secondary Institutions", "Total")
```

####2000
```{r fig.width=9, fig.height=8,eef_es_t_2000,warning=FALSE}
educationExpenditureRatioYear(2000,"Elementary and Secondary Institutions","Total")
```

####2005
```{r fig.width=9, fig.height=8,eef_es_t_2005,warning=FALSE}
educationExpenditureRatioYear(2005,"Elementary and Secondary Institutions","Total")
```

####2009
```{r fig.width=9, fig.height=8,eef_es_t_2009,warning=FALSE}
educationExpenditureRatioYear(2009,"Elementary and Secondary Institutions","Total")
```

####2010
```{r fig.width=9, fig.height=8,eef_es_t_2010,warning=FALSE}
educationExpenditureRatioYear(2010,"Elementary and Secondary Institutions","Total")
```


####2011
```{r fig.width=9, fig.height=8,eef_es_t_2011,warning=FALSE}
educationExpenditureRatioYear(2011,"Elementary and Secondary Institutions","Total")
```
...
For Higher Education Institution, starting with 1995, the Public expenditures ratios were maximum in Finland (1.7%),
Followed by Sweden (1.6%), Norway and Canada (1.5%), Denmark (1.3%). United States was on a lower place, with only 1.1%,
preceeded by Australia, with 1.2%. Through the years from 1995 to 2011, US kept his ratio between 0.9% and 1.1%. Finland
kept his leading position, increasing the ratio to 1.9% for 2010 and 2011. Sweden, Norway and Denmark kept as well their
leading position, with values above 1.5%. Greece has an interesting apparition in 2005 with a ratio of 1.4%, one of the largest.
Israel has as well a good position, with values of ratio in the same range as US.
Between European countries with good academic performance, France has a constant good ratio around and larger than 1%, with a
record value of 1.3% in 2009.


####1995
```{r fig.width=9, fig.height=8,eef_h_pu_1995,warning=FALSE}
educationExpenditureRatioYear(1995,"Higher Education Institutions", "Public")
```

####2000
```{r fig.width=9, fig.height=8,eef_h_pu_2000,warning=FALSE}
educationExpenditureRatioYear(2000,"Higher Education Institutions","Public")
```

####2005
```{r fig.width=9, fig.height=8,eef_h_pu_2005,warning=FALSE}
educationExpenditureRatioYear(2005,"Higher Education Institutions","Public")
```

####2009
```{r fig.width=9, fig.height=8,eef_h_pu_2009,warning=FALSE}
educationExpenditureRatioYear(2009,"Higher Education Institutions","Public")
```

####2010
```{r fig.width=9, fig.height=8,eef_h_pu_2010,warning=FALSE}
educationExpenditureRatioYear(2010,"Higher Education Institutions","Public")
```


####2011
```{r fig.width=9, fig.height=8,eef_h_pu_2011,warning=FALSE}
educationExpenditureRatioYear(2011,"Higher Education Institutions","Public")
```
...
In 2011, the top is dominated by South Korea (1.9%), United States (1.8%), Chile (1.7%), Japan (1%), Australia (0.9%),
Israel (0.9%), Netherlands and Russia (0.5%).

####1995
```{r fig.width=9, fig.height=8,eef_h_pr_1995,warning=FALSE}
educationExpenditureRatioYear(1995,"Higher Education Institutions", "Private")
```

####2000
```{r fig.width=9, fig.height=8,eef_h_pr_2000,warning=FALSE}
educationExpenditureRatioYear(2000,"Higher Education Institutions","Private")
```

####2005
```{r fig.width=9, fig.height=8,eef_h_pr_2005,warning=FALSE}
educationExpenditureRatioYear(2005,"Higher Education Institutions","Private")
```

####2009
```{r fig.width=9, fig.height=8,eef_h_pr_2009,warning=FALSE}
educationExpenditureRatioYear(2009,"Higher Education Institutions","Private")
```

####2010
```{r fig.width=9, fig.height=8,eef_h_pr_2010,warning=FALSE}
educationExpenditureRatioYear(2010,"Higher Education Institutions","Private")
```


####2011
```{r fig.width=9, fig.height=8,eef_h_pr_2011,warning=FALSE}
educationExpenditureRatioYear(2011,"Higher Education Institutions","Private")
```
...
The Total expenditure for Higher Education Institutions data is only available for 2011. In this year
United States tops the countries (2.7%), followed by South Korea (2.6%), Chile (2.4%), Finland (1.9%),
Denmark (1.9%), Nederland (1.8%). France and Ireland have both 1.5% while United Kingdom 1.2%.

####1995
```{r fig.width=9, fig.height=8,eef_h_t_1995,warning=FALSE}
educationExpenditureRatioYear(1995,"Higher Education Institutions", "Total")
```

####2000
```{r fig.width=9, fig.height=8,eef_h_t_2000,warning=FALSE}
educationExpenditureRatioYear(2000,"Higher Education Institutions","Total")
```

####2005
```{r fig.width=9, fig.height=8,eef_h_t_2005,warning=FALSE}
educationExpenditureRatioYear(2005,"Higher Education Institutions","Total")
```

####2009
```{r fig.width=9, fig.height=8,eef_h_t_2009,warning=FALSE}
educationExpenditureRatioYear(2009,"Higher Education Institutions","Total")
```

####2010
```{r fig.width=9, fig.height=8,eef_h_t_2010,warning=FALSE}
educationExpenditureRatioYear(2010,"Higher Education Institutions","Total")
```


####2011
```{r fig.width=9, fig.height=8,eef_h_t_2011,warning=FALSE}
educationExpenditureRatioYear(2011,"Higher Education Institutions","Total")
```
...
Let's plot the top 10 countries in parallel with top expenditure (as a percent from GDP); what we try to understand is if percent
allocated for education is enough to explain performance in education.
...
Let's merge Education expenditure per country data with UN Data showing the GDP for the countries. This will help us normalize the
expenditure per country, since the ratio that we were using until now aplies to very different GDP.
...
The result is now much closed to something we expected: Total Higher Education expenditure top includes the now allmost the same countries
as the Number of universities in CWUR per country. **USA**, **Japan**, **United Kingdom**, **France** are all in top 5 in both tops.
The countries that are in top 10 for number of universities in the CWUR and not in top of country Total Higher Education expenditure are **Israel**, **Swizerland** and **Canada**. The countries that are in the Total Higher Education expenditure top but not in the Number
of universities in CWUR top 10 are **Italy**, **Spain** and **Mexico**. **Israel**, **United Kingdom**, **Switzerland**, **Canada**
shows a very good efficiency of investment in Higher Education in terms of performance of universities.
...
Let's represent the number of universities in top per country (in 2012) as a function of Higher Education total expenses (in 2011)
per country (we already did see that these values are highly correlated).
...
Also from this representation we can notice that there is a good correlation between Higher Education total expenses and the
number of universitie in top. In terms of efficiency, United Kingdom, Israel, Norway have a higher efficiency, with a number
of universities in the World Rankings higher / Higher education total expenses compared with Japan or Italy.
...
Adding data from UNData dataset, to extract the GDP (Gross Domestic Product) allowed us to compare actuall total expenditure (by
multiplying the GDP with ratio) and we were able to see a good correlation between top 5 total expenditure for Higher Education / country
and the number of universities / country.  
